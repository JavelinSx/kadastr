...
class startWindow(QtWidgets.QMainWindow, ui.newForm.Ui_MainWindow):
    ...
    def getAllRecord(self):
        ...
        self.cursor.execute("select completerAddress from completer_data")
        completerAddress = self.cursor.fetchall()
        for item in completerAddress:
            self.address.append(item[0])

        self.cursor.execute("select completerCity from completer_data")
    ...
    def updateInfo(self):
        try:
            ...
            QMessageBox.critical(self, "Ошибка ", str("Для изменения информации, выберите клиента из базы данных"), QMessageBox.Ok)
